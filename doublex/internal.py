...
class Method(Observable):
    ...
    def __call__(self, *args, **kargs):
        ...
        retval = self.double._doublex.manage_invocation(invocation)

        if not self.double._doublex.setting_up:
            self._event.set()
    ...
    def calls(self):
        ...
        return [x._context for x in self.double._doublex.get_invocations_to(self.name)]

    def _was_called(self, context, times):
        invocation = Invocation(self.double, self.name, context)
        return self.double._doublex.received_invocation(invocation, times)
    ...
    def __repr__(self):
        return "%s.%s" % (self.double._doublex.classname(), self.name)

    def _show_history(self):
        method = "method '%s.%s'" % (self.double._doublex.classname(), self.name)
        invocations = self.double._doublex.get_invocations_to(self.name)
        if not invocations:
...
class Invocation(object):
    def __init__(self, double, name, context=None):
        ...
        self._context.signature = double._doublex.proxy.get_signature(name)
    ...
    def times(self, n):
        ...
        for i in range(1, n):
            self._double._doublex.manage_invocation(self)
    ...
    def _apply_on_collaborator(self):
        return self._double._doublex.proxy.perform_invocation(self)

    def __eq__(self, other):
        return self._double._doublex.proxy.same_method(self._name, other._name) and \
    ...
    def __repr__(self):
        return "%s.%s%s" % (self._double._doublex.classname(), self._name, self._context)
...
class PropertyGet(PropertyInvocation):
    ...
    def _apply_on_collaborator(self):
        return getattr(self._double._doublex.proxy.collaborator, self._name)

    def __repr__(self):
        return "get %s.%s" % (self._double._doublex.classname(), self._name)
...
class PropertySet(PropertyInvocation):
    ...
    def _apply_on_collaborator(self):
        return setattr(self._double._doublex.proxy.collaborator, self._name, self.value)

    def __repr__(self):
        return "set %s.%s to %s" % (self._double._doublex.classname(),
                                    self._name, self.value)


def property_factory(double, key):
    def manage(invocation):
        return double._doublex.manage_invocation(invocation)

    def get_property(obj):
        return manage(PropertyGet(double, key))

    def set_property(obj, value):
        prop = double._doublex.proxy.get_class_attr(key)
        if prop.fset is None:
            raise AttributeError("can't set attribute %s" % key)

        invocation = manage(PropertySet(double, key, value))

        if double._doublex.setting_up:
...
class AttributeFactory(object):
    ...
    def create(cls, double, key):
        get_actual_attr = lambda double, key: double._doublex.proxy.get_attr(key)
        typename = double._doublex.proxy.get_attr_typename(key)
